// src/pages/membre-direction/CreationStagiaires.jsx
import { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { UserPlus, Users, AlertCircle, Search, RefreshCw, Trash2, Filter, Check, X, ChevronDown, Mail, Phone, Book } from 'lucide-react';
import MembreDirectionLayout from '../../components/layout/MembreDirectionLayout';
import { stagiaireAPI } from '../../utils/stagiaireAPI';

export default function CreationStagiaires() {
  ...
  const fetchStagiaires = async () => {
    try {
      setIsLoading(true);
      setError('');
      const data = await stagiaireAPI.getAllStagiaires();
      // Normaliser les noms de propriétés pour s'assurer que estActif est en camelCase
      const normalisedData = data.map(stagiaire => ({
        ...stagiaire,
        // S'assurer que estActif est un booléen
        estActif: Boolean(stagiaire.estActif || stagiaire.EstActif || false)
      }));
      setStagiaires(normalisedData);
    } catch (err) {
      console.error("Erreur lors de la récupération des stagiaires:", err);
      setError("Impossible de charger la liste des stagiaires. Veuillez réessayer.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Êtes-vous sûr de vouloir supprimer ce stagiaire ? Cette action est irréversible.")) {
      try {
        setIsLoading(true);
        await stagiaireAPI.deleteStagiaire(id);
  ...
  // Fonction pour activer/désactiver un stagiaire
  const toggleStatus = async (id) => {
    try {
      setIsLoading(true);
      await stagiaireAPI.toggleStatus(id);
      // Rafraîchir la liste après mise à jour
      await fetchStagiaires();
    } catch (err) {
      console.error("Erreur lors du changement de statut:", err);
      setError("Erreur lors du changement de statut du stagiaire.");
    } finally {
      setIsLoading(false);
    }
  };

  // Filtrer les stagiaires selon le terme de recherche et le filtre de statut
  const filteredStagiaires = stagiaires.filter(stagiaire => {
    const searchMatch = 
      ...
      stagiaire.universite?.toLowerCase().includes(searchTerm.toLowerCase());
    
    if (statusFilter === 'all') return searchMatch;
    if (statusFilter === 'active') return searchMatch && stagiaire.estActif;
    if (statusFilter === 'inactive') return searchMatch && !stagiaire.estActif;
    
    return searchMatch;
  });

  return (
    <MembreDirectionLayout defaultActivePage="creation-stagiaire">
      <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100">
        ...
        <div className="p-8">
          {activeTab === 'form' ? (
            <div>
              <div className="mb-8">
                <h1 className="text-3xl font-bold mb-3 text-gray-800">Création d'un nouveau stagiaire</h1>
                <p className="text-gray-600">Remplissez le formulaire pour ajouter un nouveau stagiaire</p>
              </div>
                ...
                <div className="flex">
                  ...
                  <div className="ml-4">
                    <p className="text-green-800">
                      Pour ajouter un nouveau stagiaire, veuillez créer un compte. Le stagiaire recevra un email pour activer son compte.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex justify-center py-10">
                <button 
                  onClick={() => navigate('/auth/signup-stagiaire')} 
            ...
            <div>
              <div className="mb-6">
                <h1 className="text-3xl font-bold mb-3 text-gray-800">Liste des stagiaires</h1>
                <p className="text-gray-600">Gérez les stagiaires existants</p>
              </div>
                  ...
                  filteredStagiaires.length > 0 ? (
                    <div className="divide-y divide-gray-200">
                      {filteredStagiaires.map((stagiaire) => (
                        <div key={stagiaire.id} className="hover:bg-gray-50 transition-colors duration-150 p-5">
                          <div className="flex flex-col md:flex-row md:items-center justify-between">
                            <div className="flex flex-col mb-4 md:mb-0">
                              <div className="flex items-center">
                                ...
                                <div className="ml-4">
                                  <h4 className="text-lg font-semibold text-gray-800">{stagiaire.nom} {stagiaire.prenom}</h4>
                                  <div className="mt-1 flex items-center">
                                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${stagiaire.estActif ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                                      {stagiaire.estActif ? <Check className="w-3 h-3 mr-1" /> : <X className="w-3 h-3 mr-1" />}
                                      {stagiaire.estActif ? 'Actif' : 'Inactif'}
                                    </span>
                                    <span className="ml-3 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                      <Book className="w-3 h-3 mr-1" />
                                      {stagiaire.specialite || 'Non spécifié'}
                                    </span>
                                  </div>
                                </div>
                              </div>
                            ...
                            <div className="flex flex-col space-y-2 md:space-y-0 md:flex-row md:space-x-4">
                              ...
                              {stagiaire.universite && (
                                <div className="flex items-center text-gray-600">
                                  ...
                                  <span className="text-sm">{stagiaire.universite}</span>
                                </div>
                              )}
                            </div>
                          </div>
                          <div className="mt-4 flex justify-end space-x-3">
                            <button 
                              ...
                              onClick={() => toggleStatus(stagiaire.id)}
                            >
                              {stagiaire.estActif ? 'Désactiver' : 'Activer'}
                            </button>
                            <button 
                    ...
                    <div className="py-20 text-center">
                      ...
                      <h3 className="text-lg font-medium text-gray-700">Aucun stagiaire trouvé</h3>
                      <p className="text-gray-500 mt-2">Ajoutez des stagiaires ou modifiez vos critères de recherche</p>
                    </div>
                  )
                )}
              </div>
