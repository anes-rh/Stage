...
import { Search, AlertCircle, X, FileText, Calendar, User, Filter, ChevronDown } from 'lucide-react';
  ...
  <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
    <div className="p-4">
      <div className="flex justify-between items-start mb-3">
        ...
        <div className={`text-xs px-2 py-1 rounded ${fiche.estValide ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
      ...
      <div className="flex justify-end pt-2 border-t border-gray-100">
        <Link 
          to={`/membredirection/fiches-pointage1/${fiche.id}`}
...
const EmptyState = ({ noFiches, hasSearch }) => (
  <div className="bg-white rounded-lg shadow-sm p-8 text-center">
    ...
    <p className="text-gray-600">
      {noFiches 
        ? "Il n'y a pas encore de fiches de pointage dans le système."
...
export default function ListeFichesPointageDirecteur() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const fichesData = await fichePointageAPI.getAllFichesPointage();
  ...
  const fichesFiltrees = useMemo(() => {
    let filteredFiches = [...fichePointages];
  ...
  }, [fichePointages, filtreSearch, filtreValidation, stages]);
  ...
  if (loading) {
    return (
      <MembreDirectionLayout defaultActivePage="fiches-pointage1">
