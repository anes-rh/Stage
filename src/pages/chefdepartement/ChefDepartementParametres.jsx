// src/pages/chefdepartement/ChefDepartementParametres.jsx
...
export default function ChefDepartementParametres() {
  ...
  // Charger les données du chef de département au chargement du composant
useEffect(() => {
  const fetchUserData = async () => {
    try {
      ...
      // Extraction plus robuste du nom du département
      let departementNom = 'Non défini';
      
      // Vérifier toutes les possibilités de structure pour le département
      if (userData.departement && userData.departement.nom) {
        departementNom = userData.departement.nom;
      } else if (userData.departementNom) {
        departementNom = userData.departementNom;
      }
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      await chefdepartementAPI.updatePassword(
  ...
  // Gérer l'édition des champs (uniquement email et téléphone)
  ...
  const handleEditSubmit = async (field) => {
    try {
      // Vérifier si le champ est modifiable
      if (field !== 'email' && field !== 'telephone') {
        setUpdateError("Ce champ n'est pas modifiable");
        return;
      }
      
      // Créer l'objet de mise à jour avec uniquement le champ modifié
      const updateData = { [field]: editFormData[field] };
      
      // Appel API pour mettre à jour le champ
      await chefdepartementAPI.updateProfile(updateData);
      ...
      setUpdateSuccess(`${fieldLabels[field]} mis à jour avec succès!`);
      setEditingField(null);
    } catch (err) {
      console.error(`Erreur lors de la mise à jour du ${field}:`, err);
      setUpdateError(`Impossible de mettre à jour le champ`);
    }
  };
 ...
 // Fonction de déconnexion mise à jour
  ...
  // Fonction pour déterminer si un champ est éditable
  const isEditable = (field) => {
    return field === 'email' || field === 'telephone';
  };
  return (
    <ChefDepartementLayout defaultActivePage="parametres">
      <div className="max-w-3xl mx-auto">
                ...
                <div className="px-6 py-6 bg-gray-50 border-t border-gray-100">
                  <div className="space-y-5">
                    <div className="grid grid-cols-2 gap-5">
                      <div>
                        ...
                        <div className="w-full px-4 py-2.5 border border-gray-300 bg-gray-100 rounded-md text-sm flex justify-between items-center">
                          ...
                          <Lock className="h-4 w-4 text-gray-400" title="Ce champ n'est pas modifiable" />
                        </div>
                      </div>
                      <div>
                    ...
                    <div className="grid grid-cols-2 gap-5">
                      ...
                      <div>
                        <label className="block text-xs font-medium text-gray-700 mb-1.5">Département</label>
                        <div className="w-full px-4 py-2.5 border border-gray-300 bg-gray-100 rounded-md text-sm flex justify-between items-center">
                          <span>{userData.departementNom}</span>
                          <Lock className="h-4 w-4 text-gray-400" title="Seul l'administrateur peut modifier ce champ" />
                        </div>
                      </div>
                    </div>
                  </div>
        ...
        <div className="flex justify-between items-center">
          <button 
            ...
            onClick={() => navigate('/chefdepartement/accueil')}
          >
