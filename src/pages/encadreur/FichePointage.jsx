...
export default function FichePointage() {
    ...
    const fetchFichePointage = async () => {
      try {
        ...
        if (pointageData.length === 0) {
          // Si aucune donnée de pointage n'existe encore, initialiser avec les mois de la période de stage
          ...
          preparedPointageMois = pointageData.map(mois => {
            // S'assurer que les joursPresence existent et sont correctement formatés
        ...
        // Sélectionner le mois courant ou le premier mois disponible
  ...
  const getJourSemaineFromNumber = (number) => {
    ...
    return jourNames[number] || 'lundi'; // Valeur par défaut si number est invalide
  };
  ...
  const isDateInStagePeriod = (date) => {
    ...
    // Réinitialiser les heures pour une comparaison fiable des dates
  ...
  const isWeekend = (jourSemaine) => {
    if (typeof jourSemaine === 'number') {
      return jourSemaine === 0 || jourSemaine === 6; // Dimanche ou Samedi
    } else if (typeof jourSemaine === 'string') {
      return ["dimanche", "samedi"].includes(jourSemaine.toLowerCase());
    }
    return false;
  };
