// src/pages/encadreur/EncadreurParametres.jsx
...
export default function EncadreurParametres() {
  ...
  // Nouvel état pour l'animation de déconnexion
  const [isLoggingOut, setIsLoggingOut] = useState(false);
  
  const navigate = useNavigate();

  // Charger les données de l'encadreur au chargement du composant
useEffect(() => {
  const fetchUserData = async () => {
    try {
      ...
      // Récupérer les noms de département et de domaine séparément si nécessaire
      const departementNom = userData.departement && userData.departement.nom 
        ? userData.departement.nom 
        : (userData.departementNom || 'Non assigné');
      
      const domaineNom = userData.domaine && userData.domaine.nom 
        ? userData.domaine.nom 
        : (userData.domaineNom || 'Non assigné');
      ...
      setUserData({
        ...
        departementId: userData.departementId || (userData.departement ? userData.departement.id : null),
        domaineId: userData.domaineId || (userData.domaine ? userData.domaine.id : null)
      });
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      await encadreurAPI.updatePassword(
  ...
  const handleEditSubmit = async (field) => {
    try {
      ...
      await encadreurAPI.updateEncadreur(userData.id, { [field]: editFormData[field] });
  ...
  return (
    <EncadreurLayout defaultActivePage="parametres">
      <div className="max-w-3xl mx-auto">
                ...
                <div className="px-6 py-6 bg-gray-50 border-t border-gray-100">
                  <div className="space-y-5">
                    ...
                    <div className="grid grid-cols-2 gap-5">
                      ...
                      <div>
                        <label className="block text-xs font-medium text-gray-700 mb-1.5">Fonction</label>
                        <div className="w-full px-4 py-2.5 border border-gray-300 bg-gray-100 rounded-md text-sm flex justify-between items-center">
                    ...
                    <div className="grid grid-cols-2 gap-5">
                      <div>
                        ...
                        <div className="w-full px-4 py-2.5 border border-gray-300 bg-gray-100 rounded-md text-sm flex justify-between items-center">
                          <span>{userData.departement?.nom}</span>
                          <Lock className="h-4 w-4 text-gray-400" />
                        </div>
                      </div>
                      <div>
                        <label className="block text-xs font-medium text-gray-700 mb-1.5">Domaine</label>
                        <div className="w-full px-4 py-2.5 border border-gray-300 bg-gray-100 rounded-md text-sm flex justify-between items-center">
                          <span>{userData.domaine?.nom}</span>
                          <Lock className="h-4 w-4 text-gray-400" />
                        </div>
                      </div>
                    </div>
        ...
        <div className="flex justify-between items-center">
          <button 
            ...
            onClick={() => navigate('/encadreur/accueil')}
          >
